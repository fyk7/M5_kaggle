...
def get_data_by_store():
    ...
    cat_int_cols = ['tm_d', 'tm_w', 'tm_m', 'tm_y', 'tm_wm', 'tm_dw', 'tm_w_end']
...
############################## defaultの設定 ################################
remove_features = ['id','state_id','store_id','price_nunique', 'item_nunique', 'price_nunique'
...
def create_model(lr=0.002):
    ...
    #release_emb = Flatten()(Embedding(260,3)(release_input))
    #price_nunique_emb = Flatten()(Embedding(22,1)(price_nunique_input))
    #item_nunique_emb = Flatten()(Embedding(185, 3)(item_nunique_input))
...
model_name = '../data/output/cat_emb_model_'+'_v'+str(VER)+'.h5'
...
logger.info(f'keep features with _tmp_ : {MODEL_FEATURES}')
...
for PREDICT_DAY in range(1,29):    
    ...
    model_path = '../data/output/cat_emb_model_'+'_v'+str(VER)+'.h5' 
    estimator = keras.models.load_model(model_path)

    day_mask = base_test['d']==(END_TRAIN+PREDICT_DAY)
    #store_mask = base_test['store_id']==store_id
    
    mask = day_mask#(day_mask)&(store_mask)
    X_test = make_X(grid_df[mask][MODEL_FEATURES])
    base_test[TARGET][mask] = estimator.predict(X_test).reshape(-1)
